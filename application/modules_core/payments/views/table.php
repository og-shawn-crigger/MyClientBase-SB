<table style="width: 100%;">
    <tr>
		<?php if (isset($sort_links) AND $sort_links == TRUE) { ?>
			<th scope="col" class="first"><?php echo $table_headers['date']; ?></th>
	...
	<?php foreach ($payments as $payment) {
		if(!uri_assoc('payment_id') OR uri_assoc('payment_id') <> $payment->payment_id) { ?>
			<tr class="hoverall">
				<td class="first"><?php echo format_date($payment->payment_date); ?></td>
				<td><?php echo anchor('invoices/edit/invoice_id/' . $payment->invoice_id, $payment->invoice_number); ?></td>
				<td><?php echo anchor('contact/details/' . $payment->client_id_key .'/'. $payment->client_id, $payment->client_name); ?></td>
