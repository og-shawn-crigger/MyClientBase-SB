        ...
        $(function() {
        	...
        	$("#LocationAvailableAttributes li, #LocationVisibleAttributes li").draggable({ 
                axis: "x",
                cursor: 'move',
                distance: 30,
...
<div id="LocationVisibleAttributes" style="float:right; display:inline; width: 48%; border: 1px solid gray; padding: 3px;">
	<h3>{t}Visible Attributes{/t}<span style="font-size: 13px;"> (found {$visible_attributes|@count})</span></h3>
	<ul id="LocationVisibleAttributes" class="connectedSortable">
	{foreach $visible_attributes as $key => $attribute_name}
		<li id="LocationVisibleAttributes_{$attribute_name}" style="margin-top: 3px; padding-bottom: 1px; margin-bottom: 5px; margin-left: 3px; margin-right: 3px; background-color: #FFF; width: 390px; border: 1px solid #e8e8e8;">
			{if $all_attributes[$attribute_name]['required'] == 1}
				{$color="red"}
			{else}
				{$color="black"}
			{/if}
			...
			{if $all_attributes[$attribute_name]['desc'] != ""}
				{t}{$all_attributes[$attribute_name]['desc']}{/t}
			{else}
...
<div style="width: 48%; border: 1px solid gray; padding: 3px;">
	<h3>{t}Available Attributes{/t}<span style="font-size: 13px;"> (found {$available_attributes|@count})</span></h3>
	<ul id="LocationAvailableAttributes" class="connectedSortable">
	{foreach $available_attributes as $attribute_name => $attribute_features}
